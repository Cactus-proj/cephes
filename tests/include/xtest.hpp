...
#define EXPECT_REL_NEAR_F64_(_y_expr, _ref_expr, _rel_tol)                                                             \
    ...
    {                                                                                                                  \
        ...
        EXPECT_NEAR((_y_expr), __xtest_y_ref__, __xtest_abs_tol) << "rel_tol = " << _rel_tol;                          \
    } while (0);
#define EXPECT_REL_NEAR_F64(_y_expr, _ref_expr) EXPECT_REL_NEAR_F64_(_y_expr, _ref_expr, xtest::RelTolF64)
...
template <typename T, std::enable_if_t<std::is_floating_point_v<T>, int> = 0> constexpr T rel_tol_default()
{
    return std::sqrt(std::numeric_limits<T>::epsilon());
};
// Real (non-floating-point) types
template <typename T, std::enable_if_t<std::is_integral_v<T>, int> = 0> constexpr T rel_tol_default()
{
    return 0;
};
...
template <typename T> bool isapprox(T x, T y, T rel_tol = rel_tol_default<T>())
{
    assert(rel_tol >= 0);
    return (x == y)
           // rel_error <= rel_tol
